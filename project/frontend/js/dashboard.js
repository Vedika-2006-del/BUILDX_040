 ...
 const create=document.getElementById('createEmergency');if(create)create.onclick=async()=>{
  try{
   ...
   const data=await MediResQ.createEmergency({
    ...
    triage:document.getElementById('triage')?.value||'URGENT',
   ...
   const ghMin=Math.round((new Date(data.emergency.goldenHourDeadline)-Date.now())/60000);
   MediResQ.toast(`Emergency saved • ambulance ${data.emergency.ambulance} dispatched • ${ghMin} min left in Golden Hour`);
